...
export function CategoryScreen({ id, query }: { id: string; query: Record<string, string | string[] | undefined> }) {
  ...
  const total = transfers.reduce((sum, t) => sum + transferUsd(t), 0);
  ...
  return (
    <div className="mx-auto flex w-full max-w-[560px] flex-col pb-8">
      <BackHeader title={def.label} backHref={productHref("/analytics")} />

      {data.failed ? (
        <div className={`${glassCard} mt-2 rounded-[18px]`}>
          <ReadFailed title="We couldn't load your activity" body="This list is worked out from your activity, and it did not answer. Nothing has changed." onRetry={data.retry} />
        </div>
      ) : !data.loaded ? (
        ...
        <>
          <div className="px-1 pb-5 pt-2">
            ...
            <p className="mt-1 text-[13px] font-semibold text-white/55">
              {transfers.length} {transfers.length === 1 ? "payment" : "payments"} · {range.label}
            </p>
          </div>

          {spendCategory ? (
            <div className="mb-4">
              <InAppNote>Change categories in the HOLD app. Budgets and your own categories live there too.</InAppNote>
            </div>
          ) : null}
          ...
          {transfers.length === 0 ? (
            <div className="flex flex-col items-center gap-2.5 py-[60px]">
              ...
              <p className="text-[14px] font-semibold text-white/55">Nothing in this category</p>
            </div>
          ) : (
            <ul>
              {transfers.map((t) => (
                <li key={t.id} className="flex items-center gap-3 border-b-[0.5px] border-white/[0.06] py-3">
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-[15px] font-bold text-white">{counterpartyLabel(t)}</span>
                  ...
                  <span className="shrink-0 text-[15px] font-bold tabular-nums text-white">{fmt(transferUsd(t))}</span>
                </li>
              ))}
            </ul>
          )}

          {a.capped ? <Note className="mt-3">Showing your most recent activity. Older transactions in a long range may not be included yet.</Note> : null}
        </>
      )}
    </div>
  );
