...
import { currentMonthRange, isMonthRange, shiftMonth, type SpendRange } from "@/lib/app/spending/range";
...
export function AnalyticsScreen({ initialTab }: { initialTab?: string }) {
  ...
  const slices: DonutSlice[] = hasSpend
    ? a.categories.map((c) => ({ id: c.id, color: c.color, pct: c.pct, label: c.def.label }))
    : showTransfersRing
      ? [{ id: "transfers", color: TRANSFER_GREY, pct: 1, label: "Transfers" }]
      : [];

  const priorLabel = isMonth ? "vs last month" : "vs prior period";
  const pctStr = (p: number | null) => (p == null ? null : `${p >= 0 ? "↑" : "↓"} ${Math.abs(Math.round(p * 100))}%`);
  ...
  return (
    <div className="mx-auto flex w-full max-w-[560px] flex-col pb-8">
      <BackHeader
        title={onRecurring ? "Recurring" : "Analytics"}
        backHref={productHref()}
        right={
          <button
            type="button"
            onClick={() => setTab(onRecurring ? "overview" : "subscriptions")}
            aria-label={onRecurring ? "Back to analytics" : "Recurring payments"}
      ...
      {data.failed ? (
        <div className={`${glassCard} mt-2 rounded-[18px]`}>
          <ReadFailed title="We couldn't load your activity" body="Analytics is worked out from your activity, and it did not answer. Nothing has changed." onRetry={data.retry} />
        </div>
      ) : !data.loaded ? (
        ...
        <>
          ...
          <div className="mb-3 mt-1 flex items-center justify-between gap-3 px-1">
            {isMonth ? <span /> : <p className="min-w-0 flex-1 truncate text-[14px] font-bold capitalize text-white/[0.85]">{range.label}</p>}
            <button
              type="button"
          ...
          >
            {isMonth ? (
              <>
                <button type="button" aria-label="Previous month" onClick={() => goMonth(-1)} className="absolute left-1 top-1/2 flex h-8 w-7 -translate-y-1/2 items-center justify-center text-white/[0.28] hover:text-white/60">
                  <Ion name="chevron-back" size={16} />
                </button>
                {!atCurrentMonth ? (
                  <button type="button" aria-label="Next month" onClick={() => goMonth(1)} className="absolute right-1 top-1/2 flex h-8 w-7 -translate-y-1/2 items-center justify-center text-white/[0.28] hover:text-white/60">
            ...
            <p className="text-[11px] font-bold tracking-[1.5px] text-white/65">{(range.label || "").toUpperCase()}</p>
            <p
            ...
            <p className="mt-0.5 text-[12px] font-semibold text-white/55">{isMonth ? "You saved" : "Saved"}</p>
            {Math.abs(a.netKeptDelta) >= 0.01 ? (
              <p className="mt-2 text-[13px] font-bold" style={{ color: a.netKeptDelta < 0 ? "rgba(255,255,255,0.6)" : GREEN }}>
                {a.netKeptDelta >= 0 ? "↑" : "↓"} {fmt(Math.abs(a.netKeptDelta))} {priorLabel}
              </p>
            ) : null}
            ...
            <div className="mt-[11px] flex justify-between gap-3">
              <div className="min-w-0">
                <p className="text-[11.5px] font-semibold text-white/55">Money in</p>
                <p className="mt-px truncate text-[15px] font-bold tracking-[-0.3px] tabular-nums text-white">{fmt(a.income)}</p>
              </div>
              <div className="min-w-0 text-right">
                <p className="text-[11.5px] font-semibold text-white/55">Spent</p>
          ...
          <div className="mt-3 grid grid-cols-2 gap-2.5">
            <Bucket label="Income" value={fmt(a.income)} delta={pctStr(a.incomeDeltaPct)} deltaGreen={(a.incomeDeltaPct ?? 0) >= 0} href={metricHref("income")} />
            <Bucket label="Spend" value={fmt(a.spend)} delta={pctStr(a.spendDeltaPct)} deltaGreen={false} href={metricHref("spend")} />
          </div>
                ...
                {hasSpend ? (
                  a.categories.map((c) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        />
                        <span className={`min-w-0 flex-1 truncate text-[13px] ${active ? "font-bold text-white" : "font-semibold text-white/[0.85]"} ${dim ? "opacity-50" : ""}`}>{c.def.label}</span>
                  ...
                  <>
                    <Link href={categoryHref("transfers")} className="-mx-1.5 flex items-center gap-2 rounded-[10px] px-1.5 py-1 hover:bg-white/[0.06]">
                      ...
                      <span className="min-w-0 flex-1 truncate text-[13px] font-semibold text-white/[0.85]">Transfers</span>
                    ...
                    <p className="mt-0.5 text-[12px] leading-[17px] text-white/55">Moving your own money — counted here, but not as spending.</p>
                  </>
                ) : (
                  <p className="mt-0.5 text-[12px] leading-[17px] text-white/55">
                    No spending yet this period. When you pay for things, they’ll break down here by category.
                  </p>
                )}
              </div>
            </div>
          ...
          <SectionCard>
            ...
            <div className="flex items-center gap-2.5 rounded-[14px] border-[0.5px] border-[rgba(74,222,128,0.22)] bg-[rgba(74,222,128,0.06)] p-3.5">
              ...
              <p className="flex-1 text-[12.5px] leading-[17px] text-white/70">Budgets and your own categories live in the HOLD app.</p>
            </div>
          </SectionCard>
          ...
          <Link href={productHref("/invest")} className={`${glassCard} mt-3 flex items-center gap-3 rounded-[18px] bg-white/[0.14] p-4 transition-colors hover:bg-white/[0.18]`}>
            ...
            <span className="min-w-0 flex-1">
              <span className="block text-[13px] font-semibold text-white/55">Assets</span>
              <span className="block truncate text-[18px] font-bold tracking-[-0.4px] text-white">Your investments</span>
            </span>
          ...
          {a.excludedInternal > 0 && hasSpend ? (
            <Note>
              {fmt(a.excludedInternal)} in transfers between your own accounts is excluded from spend — moving your money isn&apos;t income or spend.
            </Note>
          ) : null}
  ...
  if (subs.length === 0) {
    return (
      <div className="flex flex-col items-center gap-2.5 py-9 text-center">
        ...
        <p className="text-[14px] font-semibold text-white/55">No recurring payments detected yet</p>
        <p className="px-10 text-[12.5px] leading-[18px] text-white/55">Recurring charges (Netflix, gym, rent…) show up here automatically.</p>
      </div>
    );
  }
  return (
    <div>
      <div className="px-1 pb-[18px] pt-1.5">
        <p className="text-[13px] font-semibold text-white/55">Estimated monthly</p>
        ...
        <p className="mt-1 text-[12.5px] font-semibold text-white/55">
          {subs.length} {subs.length === 1 ? "payment" : "payments"} · swipe to browse
        </p>
      </div>
