...
const CONTACT_KINDS: ContactKind[] = ["telegram", "x", "email"];
...
function contactProblem(kind: ContactKind, value: string): string | null {
  const v = value.trim();
  if (!v) return `Add your ${CONTACT_KIND_LABEL[kind]} so the creator can reach you.`;
...
function normaliseContact(kind: ContactKind, value: string): string {
  const v = value.trim();
