      ...
      try {
        ...
        if (existing) {
          ...
          if (own) {
            ...
            // DEMO BRANCH: ?step= opens the draft on that card. The editor is
            // a pager now, so the step's name is turned into its position --
            // and a saved draft has no product card, so the picker is dropped
            // first, exactly as the pager itself drops it.
            const want = demoParam("step");
            const wanted = stagesFor(own).slice(1).findIndex((st) => st.key === want);
            if (wanted >= 0) setAt(wanted);
          }
        }
      } catch (e) {
